################################################################################
# Single-Cell RNA-seq Analysis with Cell Hashing - Batch 4
# 
# Description: Processing and demultiplexing of single-cell RNA-seq data using
# hashtag oligonucleotides (HTOs) for sham and injured samples (Batch 4)
#
# Requirements: 
...
shamB4_cdna_path <- "Data_Files/cDNA/KS230616_Sham_filtered_feature_bc_matrix.h5"
injuryB4_cdna_path <- "Data_Files/cDNA/KS230616_Injury_filtered_feature_bc_matrix.h5"

# Path to HTO count data (kallisto/BUStools output)
shamB4_hto_path <- "Data_Files/HTOs/KS230616_sham_counts_unfiltered"
injuryB4_hto_path <- "Data_Files/HTOs/KS230616_Undetermined_counts_unfiltered"

################################################################################
# PROCESS SHAM SAMPLES - BATCH 4
################################################################################

cat("=== Processing Sham Samples (Batch 4) ===\n")

# Load cDNA data
ShamB4.umis <- Read10X_h5(shamB4_cdna_path)
cat("Sham B4 cDNA dimensions:", dim(ShamB4.umis), "\n")

# Load HTO data
ShamB4.htos <- BUSpaRse::read_count_output(shamB4_hto_path, 
                                           name = "cells_x_features", 
                                           tcc = FALSE)
cat("Sham B4 HTO dimensions:", dim(ShamB4.htos), "\n")
cat("HTO names:\n")
print(head(ShamB4.htos, n = 6))
print(rownames(ShamB4.htos))

# Join the RNA and HTO
# Rename UMI columns to match HTO (remove -1 suffix)
cat("\nBefore modification:\n")
cat("cDNA barcodes:", head(colnames(ShamB4.umis)), "\n")
cat("HTO barcodes:", head(colnames(ShamB4.htos)), "\n")

colnames(ShamB4.umis) <- gsub("-1", "", colnames(ShamB4.umis))

cat("After modification:\n")
cat("cDNA barcodes:", tail(colnames(ShamB4.umis)), "\n")

# Join cDNA and HTO data
joint.bcsB4_Sham <- dplyr::intersect(colnames(ShamB4.htos), colnames(ShamB4.umis))
cat("Joint barcodes found:", length(joint.bcsB4_Sham), "\n")

# Subset RNA and HTO counts by joint cell barcodes
ShamB4.umis <- ShamB4.umis[, joint.bcsB4_Sham]
ShamB4.htos <- as.matrix(ShamB4.htos[, joint.bcsB4_Sham])
cat("Final Sham B4 HTO dimensions:", dim(ShamB4.htos), "\n")
...
ShamB4.hashtag <- NormalizeData(ShamB4.hashtag, assay = "HTO", 
...
cat("\nSham B4 demultiplexing results:\n")
print(table(ShamB4.hashtag$MULTI_ID))

# Group cells based on the max HTO signal
Idents(ShamB4.hashtag) <- "MULTI_ID"

# Visualizations
VlnPlot(ShamB4.hashtag, assay = "HTO", 
        features = rownames(ShamB4.hashtag[["HTO"]])[1:4], ncol = 2)
RidgePlot(ShamB4.hashtag, assay = "HTO", 
          features = rownames(ShamB4.hashtag[["HTO"]])[1:4], ncol = 2)
...
cells_to_plot__ShamB4.hashtag <- sample(colnames(ShamB4.hashtag), 
                                        min(5000, ncol(ShamB4.hashtag)))
...
pheatmap(hto_data_subset__ShamB4.hashtag, cluster_rows = FALSE, cluster_cols = FALSE,
         show_colnames = FALSE, 
         annotation_col = data.frame(ID = ShamB4.hashtag$MULTI_ID[cells_ordered__ShamB4.hashtag],
                                     row.names = cells_ordered__ShamB4.hashtag))

################################################################################
# PROCESS INJURED SAMPLES - BATCH 4
################################################################################

cat("\n=== Processing Injured Samples (Batch 4) ===\n")

# Read in the cDNA
InjuryB4.umis <- Read10X_h5(injuryB4_cdna_path)
cat("Injury B4 cDNA dimensions:", dim(InjuryB4.umis), "\n")

# Read in the hashtags
InjuryB4.htos <- BUSpaRse::read_count_output(injuryB4_hto_path, 
                                             name = "cells_x_features", 
                                             tcc = FALSE)
cat("Injury B4 HTO dimensions:", dim(InjuryB4.htos), "\n")
cat("HTO names:\n")
print(head(InjuryB4.htos, n = 4))
print(rownames(InjuryB4.htos))

# Join the RNA and HTO
# Rename UMI columns to match HTO
cat("\nBefore modification:\n")
cat("cDNA barcodes:", head(colnames(InjuryB4.umis)), "\n")
cat("HTO barcodes:", head(colnames(InjuryB4.htos)), "\n")

colnames(InjuryB4.umis) <- gsub("-1", "", colnames(InjuryB4.umis))

cat("After modification:\n")
cat("cDNA barcodes:", tail(colnames(InjuryB4.umis)), "\n")

# Join cDNA and HTO data
joint.bcs_injuryB4 <- dplyr::intersect(colnames(InjuryB4.htos), colnames(InjuryB4.umis))
cat("Joint barcodes found:", length(joint.bcs_injuryB4), "\n")

# Subset RNA and HTO counts by joint cell barcodes
InjuryB4.umis <- InjuryB4.umis[, joint.bcs_injuryB4]
InjuryB4.htos <- as.matrix(InjuryB4.htos[, joint.bcs_injuryB4])
cat("Final Injury B4 HTO dimensions:", dim(InjuryB4.htos), "\n")
cat("HTO row names confirmed:\n")
print(rownames(InjuryB4.htos))
...
# Normalize HTO data, here we use centered log-ratio (CLR) transformation
InjuryB4_hashtag <- NormalizeData(InjuryB4_hashtag, assay = "HTO", 
                                  normalization.method = "CLR", margin = 2)

# Demultiplex the data
InjuryB4_hashtag <- MULTIseqDemux(InjuryB4_hashtag, assay = "HTO", autoThresh = TRUE)
cat("\nInjury B4 demultiplexing results:\n")
print(table(InjuryB4_hashtag$MULTI_ID))

# Group cells based on the max HTO signal
Idents(InjuryB4_hashtag) <- "MULTI_ID"

# Visualizations
VlnPlot(InjuryB4_hashtag, assay = "HTO", 
        features = rownames(InjuryB4_hashtag[["HTO"]])[1:4], ncol = 2)
RidgePlot(InjuryB4_hashtag, assay = "HTO", 
          features = rownames(InjuryB4_hashtag[["HTO"]])[1:4], ncol = 2)
...
# 2. Sample cells (to avoid plotting too many)
set.seed(42) # For reproducibility
cells_to_plot__InjuryB4_hashtag <- sample(colnames(InjuryB4_hashtag), 
                                          min(5000, ncol(InjuryB4_hashtag)))
...
pheatmap(hto_data_subset__InjuryB4_hashtag, cluster_rows = FALSE, cluster_cols = FALSE,
         show_colnames = FALSE, 
         annotation_col = data.frame(ID = InjuryB4_hashtag$MULTI_ID[cells_ordered__InjuryB4_hashtag],
...
# Check that overlapping cell barcodes have been removed
...
Sham.singletB4@meta.data$orig.ident <- plyr::mapvalues(x = Sham.singletB4@meta.data$orig.ident, 
...
cat("Cells before QC filtering:", length(WhichCells(Sham.singletB4)), "\n")
...
# Visualize results
VlnPlot(Sham.singletB4, features = c("nFeature_RNA", "nCount_RNA", "percent.mt"), 
        ncol = 3, group.by = "orig.ident")
cat("Cells after QC filtering:", length(WhichCells(Sham.singletB4)), "\n")
cat("Sham B4 samples by HTO:\n")
print(table(Sham.singletB4$MULTI_ID))

################################################################################
# EXTRACT SINGLETS AND PERFORM QC - INJURED SAMPLES
################################################################################

cat("\n=== Processing Injured B4 Singlets ===\n")
...
InjuryB4.singlet@meta.data$orig.ident <- plyr::mapvalues(x = InjuryB4.singlet@meta.data$orig.ident, 
                                                         from = current.project.id, 
                                                         to = new.project.id)
...
cat("Cells before QC filtering:", length(WhichCells(InjuryB4.singlet)), "\n")
VlnPlot(InjuryB4.singlet, features = c("nFeature_RNA", "nCount_RNA", "percent.mt"), 
        ncol = 3, group.by = "orig.ident")
...
# Visualize results
VlnPlot(Injury.singletB4, features = c("nFeature_RNA", "nCount_RNA", "percent.mt"), 
        ncol = 3, group.by = "orig.ident")
VlnPlot(Injury.singletB4, features = c("nFeature_RNA", "nCount_RNA", "percent.mt"), ncol = 3)
cat("Cells after QC filtering:", length(WhichCells(Injury.singletB4)), "\n")
cat("Injured B4 samples by HTO:\n")
print(table(Injury.singletB4$MULTI_ID))
cat("Injury.singletB4 dimensions:", dim(Injury.singletB4), "\n")

cat("\n=== Processing Complete ===\n")
cat("Final objects created:\n")
cat("  - Sham.singletB4: QC-filtered sham B4 singlets\n")
cat("  - Injury.singletB4: QC-filtered injured B4 singlets\n")